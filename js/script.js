const global = {
  ...
  api: {
    API_KEY: "3b578ffe124f90c0af6160eca84b8883",
...
async function displayPopularMovies() {
  ...
  results.forEach((movie) => {
    ...
    ${
      movie.poster_path
        ? `<img
      src="https://image.tmdb.org/t/p/w500.jpg${movie.poster_path}"
      class="card-img-top"
      alt="${movie.title}"
    />`
        : `<img
...
<div class="details-bottom">
  ...
  ${movie.production_companies
    ...
    .map((companies) => `<span>  ${companies.name}  </span>`)
    .join(", ")}</div>
</div>`;
...
async function displayShowdetails() {
  ...
  ${
    show.poster_path
      ? `<img
      src="https://image.tmdb.org/t/p/w500.jpg${show.poster_path}"
      class="card-img-top"
      alt="${show.name}"
    />`
      : `<img
...
async function search() {
  ...
  if (global.search.term !== "" && global.search.term !== null) {
    const { results, total_pages, page, total_results } = await serchAPIData();
    const demo = await serchAPIData();
    console.log(demo);
...
function displaySearchResults(results) {
  ...
  <div class="card-body">
    ...
    ).innerHTML = `<h2>${results.length} of ${global.search.totalResult} Results for ${global.search.term}</h2>`;
...
function displayPagination() {
  ...
  <div class="page-counter">Page ${global.search.page} of ${global.search.totalResult}</div>
</div>`;
  ...
  document.querySelector("#next").addEventListener("click", async () => {
    global.search.page++;
    const { results, total_pages } = await serchAPIData();
    displaySearchResults(results);
  });
...
// Fuction for adding comma

function addCommasToNumbers(number) {
  return number.toLocaleString();
}

// Initializtion app and Routes
